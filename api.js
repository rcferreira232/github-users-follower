...
async function listarTodosSeguidores() {
  try {
    ...
    while (true) {
      ...
      seguidores = seguidores.concat(
        response.data.map((seguidor) => seguidor.login)
      )
...
async function listarTodosSeguindo() {
  try {
    ...
    while (true) {
      ...
      seguindo = seguindo.concat(
        response.data.map((seguindo) => seguindo.login)
      )
...
Promise.all([listarTodosSeguidores(), listarTodosSeguindo()]).then(
  ([seguidores, seguindo]) => {
    console.log('Seguidores que você não está seguindo:')
    const seguidoresNaoSeguindo = seguidores.filter(
      (seguidor) => !seguindo.includes(seguidor)
    )
    console.log(seguidoresNaoSeguindo)

    console.log('Pessoas que você segue, mas não te seguem:')
    const seguindoNaoSeguidores = seguindo.filter(
      (seguindo) => !seguidores.includes(seguindo)
    )
...
//Remover pessoas que você segue, mas não te seguem
async function removerSeguindoNaoSeguidores() {
  try {
    const seguidores = await listarTodosSeguidores()
    const seguindo = await listarTodosSeguindo()

    const seguindoNaoSeguidores = seguindo.filter(
      (seguindo) => !seguidores.includes(seguindo)
    )

    for (const usuario of seguindoNaoSeguidores) {
      await axios.delete(`https://api.github.com/user/following/${usuario}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })

      console.log(`Você deixou de seguir ${usuario}`)
    }
  } catch (error) {
    console.error('Erro ao remover quem você está seguindo:', error.message)
  }
}

removerSeguindoNaoSeguidores()
